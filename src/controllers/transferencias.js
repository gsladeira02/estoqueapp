...
async function solicitar(req, res) {
  ...
  // Busca dados do produto para checar conversão
  const { data: produto } = await supabase
    .from('produtos')
    .select('tipo, unidade, unidade_insumo, fator_conversao')
    .eq('id', produto_id)
    .single()

  // Se produto é "ambos" e será transferido como insumo, exige finalidade
  ...
  // Verifica saldo na origem (sempre em unidade original)
  ...
  // Calcula quantidade no destino (com conversão se for insumo)
...
async function resolver(req, res) {
  ...
  if (acao === 'aprovar') {
    ...
    // Saída na origem (unidade original)
    ...
    // Entrada no destino
    // Se houve conversão (finalidade materia_prima com fator), registra em unidade_destino
    ...
    await supabase.from('movimentacoes').insert({
      ...
      documento: id,
      finalidade: transferencia.finalidade,
      // unidade da movimentação segue a unidade_destino
      custo_unitario: null
    })
  }
